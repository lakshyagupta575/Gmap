...
function pump() {
  ...
  if ($('select[name="mode"').val() == "Car") {
    $("#petrol").append("<p></p>");
    $("#petrol p").append(
      "<span class = ''>Do you want to travel on a route comprised of Petrol Pumps?  </span>"
    );
    $("#petrol p").append(
      "<label><input class='ml-2' type='radio' name='pumps' value='yes'> Yes </label>"
    );
    $("#petrol p").append(
      "<label><input class='ml-2' type='radio' name='pumps', value='no'> No</label>"
    );
  } else {
    $("#petrol").html("");
  }
...
function solve(graph, city) {
  ...
  while (true) {
    ...
    for (var n in solutions) {
      ...
      var ndist = solutions[n].dist; // gets distance to the nearest vertext to the source vertex(initially)...after that closest to the next select closest vertex
      //console.log(ndist);
      var adj = graph[n]; // gets adjacent nodes to the selected nearest node to the source vertex(initially)
      //console.log(adj);
...
function find(mode, from, to) {
  ...
  $("#distance").append("<p></p>");
  $("#distance p").append(
    "<strong>Distance: &nbsp;</strong>" + dist.dist + " km"
  );

  $("#restaurant").append("<p></p>");
  $("#restaurant p").append(
    "<span class = 'spanny'>Do you want to check out famous Restaurants of this city?  </span>"
  );
  $("#restaurant p").append(
    "<label class = 'but1'><input class='ml-2' type='radio' name='restro' value='yes'> Yes </label>"
  );
  $("#restaurant p").append(
    "<label class = 'but1'><input class='ml-2' type='radio' name='restro' value='no'> No</label>"
  );
}
